...
async def pspam(event):
    if event.sender_id in SUDO_USERS:
        if event.chat_id in GROUP:
            await event.reply("» YE GROUP 𓆩🜲𓆪KILLER𓆩🜲𓆪 KE UNDER MAI HAI ISLEYE ISME PSPAM NHI HOGA...")
        else:
            try:
...
async def hang(e):
    if e.sender_id in SUDO_USERS:
        if e.chat_id in GROUP:
            await e.reply("» YE GROUP 𓆩🜲𓆪KILLER𓆩🜲𓆪 KE UNDER MAI HAI ISLEYE ISME HANG NHI HOGA..")
        else:
            try:
